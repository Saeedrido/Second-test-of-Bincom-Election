...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Dashboard'); ?> — INEC Election Dashboard</title>
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-[280px] bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-300 ease-out lg:translate-x-0"
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1 scrollbar-thin" aria-label="Sidebar">
                ...
                <?php $__currentLoopData = $navItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $active = Route::currentRouteNamed($item['route']); ?>
                    <a href="<?php echo e(route($item['route'])); ?>"
                       ...
                       aria-current="<?php echo e($active ? 'page' : null); ?>">
                        <i data-lucide="<?php echo e($item['icon']); ?>" class="w-[18px] h-[18px] shrink-0 <?php echo e($active ? 'text-brand-600 dark:text-brand-400' : 'text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-300'); ?>" aria-hidden="true"></i>
            ...
            <header class="sticky top-0 z-30 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800" role="banner">
                <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-lg font-semibold text-slate-900 dark:text-white"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h1>
                            <?php if (! empty(trim($__env->yieldContent('subtitle')))): ?>
                                <p class="text-sm text-slate-500 dark:text-slate-400 -mt-0.5"><?php echo $__env->yieldContent('subtitle'); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
...
<?php /**PATH C:\Users\Prof. Timehin\Desktop\bincom-election-assessment\resources\views/layouts/app.blade.php ENDPATH**/ ?>