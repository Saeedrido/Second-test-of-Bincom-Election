<?php $__env->startSection('title', 'Dashboard PDF Export'); ?>

<?php $__env->startPush('header-actions'); ?>
...
<div x-data="{ loaded: false }" x-init="$nextTick(() => loaded = true)">
    ...
    <div class="mb-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800"
        ...
        <div class="text-center border-b border-slate-200 dark:border-slate-700 pb-6 mb-6">
            ...
            <p class="text-xs text-slate-400 dark:text-slate-500 mt-2">Generated on: <?php echo e(now()->format('F j, Y \a\t g:i A')); ?></p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <div class="text-center p-5 rounded-xl bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 border border-blue-200 dark:border-blue-800">
                <div class="text-3xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['total_polling_units'])); ?></div>
                <div class="text-xs text-slate-500 dark:text-slate-400 mt-2 uppercase tracking-wider">Polling Units</div>
            </div>
            <div class="text-center p-5 rounded-xl bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 border border-blue-200 dark:border-blue-800">
                <div class="text-3xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['total_lgas'])); ?></div>
                <div class="text-xs text-slate-500 dark:text-slate-400 mt-2 uppercase tracking-wider">Local Government Areas</div>
            </div>
            <div class="text-center p-5 rounded-xl bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 border border-blue-200 dark:border-blue-800">
                <div class="text-3xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['total_wards'])); ?></div>
                <div class="text-xs text-slate-500 dark:text-slate-400 mt-2 uppercase tracking-wider">Wards</div>
            </div>
            <div class="text-center p-5 rounded-xl bg-gradient-to-br from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20 border border-blue-200 dark:border-blue-800">
                <div class="text-3xl font-bold text-blue-600 dark:text-blue-400"><?php echo e(number_format($stats['total_parties'])); ?></div>
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <?php $__currentLoopData = $stats['top_parties']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $party): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            <td class="px-6 py-4 text-sm text-slate-900 dark:text-white"><?php echo e($index + 1); ?></td>
                            <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-white"><?php echo e($party->party_name); ?></td>
                            <td class="px-6 py-4 text-right text-sm font-semibold tabular-nums text-slate-900 dark:text-white"><?php echo e(number_format($party->total_score)); ?></td>
                            <td class="px-6 py-4 text-right text-sm tabular-nums text-slate-600 dark:text-slate-400"><?php echo e($pct); ?>%</td>
                        </tr>
                ...
                <tfoot>
                    <tr class="bg-slate-50 dark:bg-slate-900/50">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-bold tabular-nums text-slate-900 dark:text-white"><?php echo e(number_format($grandTotal)); ?></td>
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php $__currentLoopData = $stats['recent_results']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <?php if($pu): ?>
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                            <td class="px-6 py-4 text-sm font-medium text-slate-900 dark:text-white"><?php echo e($pu->polling_unit_name); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($pu->ward?->ward_name ?? 'N/A'); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($pu->lga?->lga_name ?? 'N/A'); ?></td>
                            <td class="px-6 py-4 text-right text-sm font-semibold tabular-nums text-slate-900 dark:text-white"><?php echo e(number_format($totalVotes)); ?></td>
                            <td class="px-6 py-4 text-right text-xs tabular-nums text-slate-400 dark:text-slate-500"><?php echo e($entry->latest_entry ? \Carbon\Carbon::parse($entry->latest_entry)->format('M d, Y') : 'N/A'); ?></td>
                        </tr>
                        <?php endif; ?>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Prof. Timehin\Desktop\bincom-election-assessment\resources\views/pdf/dashboard.blade.php ENDPATH**/ ?>