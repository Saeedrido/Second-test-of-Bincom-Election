...
@section('title', 'Add Polling Unit Results')
...
<div x-data="resultForm" x-init="$nextTick(() => loaded = true)">
    ...
    <form action="{{ route('results.store') }}" method="POST" x-ref="resultForm">
        @csrf
                    ...
                    @foreach($parties as $index => $party)
                        ...
                        <div class="group relative rounded-xl border border-slate-200 p-4 transition-all hover:border-blue-300 hover:shadow-sm dark:border-slate-700 dark:hover:border-blue-600">
                            <div class="flex items-center gap-3 mb-3">
                                ...
                                <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{ $party->partyname }}</span>
                            </div>
                            <input type="number"
                                   name="results[{{ $index }}][party_score]"
                                   min="0"
                                   value="{{ old("results.{$index}.party_score") }}"
                                   required
                                   aria-label="{{ $party->partyname }} votes"
                                   placeholder="0"
                                   @input="recalcTotal()"
                            ...
                            <input type="hidden" name="results[{{ $index }}][party_abbreviation]" value="{{ $party->partyid }}">
                        </div>
                    @endforeach
                </div>
